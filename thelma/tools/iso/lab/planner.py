...
from thelma.tools.semiconstants import get_max_dilution_factor
...
from thelma.tools.iso.lab.base import get_stock_takeout_volume
...
class LabIsoBuilder(object):
    ...
    def __init__(self, iso_request, excluded_racks, requested_tubes):
        ...
        self._exluded_racks = excluded_racks
        if len(excluded_racks) < 1: self._exluded_racks = None
        #: A list of barcodes from stock tubes that are supposed to be used
        #: (for fixed positions).
        self._requested_tubes = requested_tubes
        if len(requested_tubes) < 1: self._requested_tubes = None
    ...
    def create_isos(self):
        ...
        while len(isos) < self._isos_to_generate:
            ...
            iso = self._create_iso(iso_label)
            isos.append(iso)
    ...
    def _create_iso(self, label):
        """
        Creates an ISO including ISO plates.
        """
        if not self._floating_candidates is None:
            pool_set_type = \
                    self.iso_request.molecule_design_pool_set.molecule_type
        else:
            pool_set_type = None
        final_layout = \
            self._create_layout_without_floatings(self.final_iso_layout)
        floating_map = dict()
        pools = set()
        self.__fill_final_plate_layout(final_layout, floating_map, pools)
        pool_set = None
        if len(pools) > 0:
            pool_set = MoleculeDesignPoolSet(molecule_type=pool_set_type,
                                             molecule_design_pools=pools)
        prep_layouts = self._complete_iso_preparation_layouts(floating_map)
    ...
    def __fill_final_plate_layout(self, iso_plate_layout, floating_map, pools):
        """
    ...
    def _create_layout_without_floatings(self, template_layout):
        """
    ...
    def __add_final_iso_plates(self, iso):
        """
...
class _LocationContainer(object):
    ...
    def __init__(self, volume, target_concentration, parent_concentration,
        ...
        """
        if self.__class__ is _LocationContainer:
...
class _PlateContainer(object):
    ...
    def __init__(self, plate_marker, available_locations):
        ...
        :param str plate_marker: Contains the role and a plate number. Is
            generated by :func:`LABELS.create_plate_marker`.
        ...
        """
        if self.__class__ is _PlateContainer:
            raise NotImplementedError('Abstract class.')
        self.plate_marker = plate_marker
        #: The containers for each possible locations. Locations without
        #: containers are also part of the map; their value is *None*.
...
class RackPositionPlateContainer(_PlateContainer):
    ...
    """
    def __init__(self, plate_marker, available_locations,
                 check_preferred_locations=True):
        """
        Constructor.

        :param bool check_preferred_locations: Flag indicating if the
          container should check the preferred locations when setting new
          containers.
        """
        _PlateContainer.__init__(self, plate_marker, available_locations)
        self.__check_preferred_locations = check_preferred_locations
        if not check_preferred_locations:
            # If we do not care about preferred positions to match some given
            # layout, we create a layout that is column-first to optimize for
            # robot speed.
            # FIXME: This is a little hacky as it messes with the protected
            #        empty locations list.
            self.__optimize_available_locations()
    ...
    def _find_location(self, container, preferred_locations):
        ...
        same row.
        """
        if not self.has_empty_locations():
            msg = 'There are no empty positions left!'
            raise RuntimeError(msg)
        if self.__check_preferred_locations \
            and not preferred_locations is None:
            rack_pos = None
            for pref_loc in preferred_locations:
                if pref_loc in self._empty_locations:
                    rack_pos = pref_loc
                    break
            if rack_pos is None:
                rack_pos = self._empty_locations[0]
        else:
            # In the optimized case, just take the first location from the
            # empty locations list.
            rack_pos = self._empty_locations[0]
        return rack_pos

    def __optimize_available_locations(self):
        # Orders the available empty locations by column index.
        column_first_index_map = \
            dict([((pos.column_index, pos.row_index), pos)
                  for pos in self._empty_locations])
        self._empty_locations = \
            [it[1] for it in sorted(column_first_index_map.items())]
...
class _LocationAssigner(object):
    ...
    def __init__(self, prep_reservoir_specs, final_reservoir_specs):
        """
        Constructor.

        :param prep_reservoir_specs: Preparation plate reservoir specs to use.
        :type prep_reservoir_specs: :class:`ReservoirSpecs`
        :param final_reservoir_specs: Final plate reservoir specs to use.
        :type final_reservoir_specs: :class:`ReservoirSpecs`
        """
        if self.__class__ is _LocationAssigner:
            raise NotImplementedError('Abstract class.')
        self._prep_specs = prep_reservoir_specs
        self.__final_reservoir_specs = final_reservoir_specs
    ...
    def __requires_intermediate_position(self, target_container,
        ...
        if not target_container.allows_modification:
            if not source_container is None and \
                ...
                mdv = self.__final_reservoir_specs.min_dead_volume \
                        * VOLUME_CONVERSION_FACTOR
                if is_smaller_than(source_container.final_volume, mdv):
                    return True
    ...
    def __create_plate_container(self, plate_marker):
        ...
        kw = dict(available_locations=self.__available_prep_locations,
                  plate_marker=plate_marker,
                  check_preferred_locations=\
                    self._prep_specs.rack_shape ==
                                    self.__final_reservoir_specs.rack_shape)
...
class _LayoutPlanner(BaseTool):
    ...
    def __init__(self, iso_request, builder, parent=None):
        ...
        #: The reservoir specs of the final plate.
        self._final_reservoir_specs = None
...
class RackPositionPlanner(_LayoutPlanner):
    ...
    def _init_assigner(self, reservoir_specs):
        return self._LOCATION_ASSIGNER_CLS(reservoir_specs,
                                           self._final_reservoir_specs)
...
class LibraryIsoBuilder(LabIsoBuilder):
    ...
    def _create_iso(self, label):
        """
        Creating a Library ISO is much simpler than a standard Lab ISO
        as we do not have to deal with floating positions.
        """
    ...
    def __make_final_layout(self):
        # We need to add the library positions to the final layout (taken from
        # the __iso_request_layout; the actual pool ID for the positions is
        # not inserted).
        copied_final_layout = \
            self._create_layout_without_floatings(self.final_iso_layout)
        for ir_pos in self._iso_request_layout.get_sorted_working_positions():
            if not ir_pos.is_library:
                continue
            lib_pos = FinalLabIsoPosition.create_library_position(
                                rack_position=ir_pos.rack_position,
                                concentration=ir_pos.iso_concentration,
                                volume=ir_pos.iso_volume)
            copied_final_layout.add_position(lib_pos)

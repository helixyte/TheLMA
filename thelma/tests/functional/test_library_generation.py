...
class TestLibraryGeneration(TestFunctionalBase):
    setup_rdb_context = True
    __session = None

    def test_workflow(self, app_creator, user_cenixadm):
        repo_mgr = \
        ...
        ir = self.__generate_iso_request(lib_excel_data, user_cenixadm)
        self.__generate_isos(ir, user_cenixadm)
        self.__create_iso_job(ir, user_cenixadm, 3)
        ...
        for (iso_idx, iso) in enumerate(ir.isos):
            ...
            self.__run_worklist_writer_for_iso(iso, empty_racks,
                                               ...
                                               user_cenixadm)
        for iso in ir.isos:
            self.__run_executor_for_iso(iso, user_cenixadm)

    def __generate_iso_request(self, excel_data, requester):
        lg = LibraryCreationIsoRequestGenerator(
                                        'testlib', excel_data,
                                        requester,
    ...
    def __run_worklist_writer_for_iso(self, iso, empty_racks,
        ...
        for fn, strm in file_map.iteritems():
            if not 'xl20_dummy_output' in fn or 'Qall' in fn:
                continue

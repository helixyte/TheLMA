...
an experiment design if there was no ISO specified in an excelerator file.

ISO Parameters
..............

There are 4 different types of position for an ISO Layout:

 * empty position
 * mock position (no molecule design but all other liquids)
 * fixed position (containing a specified molecule design)
 * floating position (containing a molecule design which is not specified)

For fixed and floating position, the following parameters must be specified:
 * molecule design ID (or placeholder)

It is also possible to specify a certain vendor for a molecule design.
...
In order to generate the mastermix the following information must be known:
  ...
  * transfection reagent concentration (\'reagent concentration\')
  * molecule design ID (or placeholder)
...
:Note: The tool does not check whether all design racks are compatible to the
    ISO layout. This is taken over by the WellAssociator further downstream in
    the experiment metadata generation process.
...
__all__ = ['TransfectionLayoutFinder',
           'TransfectionTransferItem',
           'TransfectionLayoutOptimizer']
    ...
    **Return Value:** TransfectionLayout for the ISO plate (with mastermix
    ...
    def __get_experiment_layouts(self):
        ...
        for design_rack in self.experiment_design.design_racks:
            experiment_layout = design_rack.layout
    ...
    def __check_for_iso_volume_and_concentration(self, design_rack):
        ...
        for tag in design_rack.layout.get_tags():
    ...
    def __optimise_layout(self):
        ...
        optimiser = TransfectionLayoutOptimizer(log=self.log,
...
class TransfectionLayoutOptimizer(BiomekLayoutOptimizer):
    ...
    **Return Value:** The optimised transfection layout.
    ...
    TRANSFER_ITEM_CLASS = TransfectionTransferItem

...
class PoolCreationExecutor(BaseAutomationTool):
    ...
    tube handler worklists). This comprises:
        ...
        - execution of all worklist files (buffer addition and stock transfer)
    ...
    **Return Value:** the updated ISO
    ...
    def __init__(self, pool_creation_iso, user,
        ...
        #: The pool creation ISO for which to execute the worklists.
        ...
        #: The ISO sample stock rack for the ISO (that is the rack that will
        #: contain the new pool stock samples).
        self.__sample_stock_rack = None
    ...
    def get_executed_stock_worklists(self):
        ...
        return self.__exec_stock_transfer_wls
    ...
    def __get_racks(self):
        """
        Fetches the ISO sample stock rack and the single molecule stock racks
        ...
        # There should only be one ISO sample stock rack
        issrs = self.pool_creation_iso.iso_sample_stock_racks
        if not len(issrs) == 1:
            msg = 'There is an unexpected number of ISO sample stock racks ' \
                  'attached to this ISO (%i). There should be exactly one! ' \
                  % (len(issrs))
            self.add_error(msg)
            return None
        else:
            self.__sample_stock_rack = issrs[0]

        label = self.__sample_stock_rack.planned_worklist.label
    ...
    def __verify_single_md_stock_racks(self):
        ...
        stock racks.
        """
        verifier = PoolCreationStockRackVerifier(log=self.log,
...
class PoolCreationStockRackVerifier(BaseAutomationTool):
    ...
    **Return Value:** boolean
    ...
    def __check_input(self):
        ...
        self._check_input_class('pool creation ISO layout', self.library_layout,
                                LibraryLayout)
    ...
    def __get_expected_pools(self):
        ...
        """
        pool_map = dict()
        for rack_pos, lib_pos in self.library_layout.iterpositions():
            pool_map[rack_pos] = lib_pos.pool
        return pool_map

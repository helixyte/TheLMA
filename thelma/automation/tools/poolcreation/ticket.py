...
__all__ = ['PoolCreationTicketGenerator',
           'PoolCreationIsoCreator',
           'PoolCreationTicketWorklistUploader',
           'PoolCreationStockLogFileWriter',
           'PoolCreationStockTransferReporter']


class PoolCreationTicketGenerator(BaseTracTool):
    """
...
class PoolCreationIsoCreator(BaseAutomationTool):
    """
    Creates ticket and ISO for a pool stock sample creation ISO.
    ...
    **Return Value:** The updated ISO request (incl. ISOs).
    ...
    #: Name pattern for pool creation ISO labels. The placeholders are
    #: the ISO request name and the layout number.
    ...
    def __create_isos(self):
        ...
        ir_label = self.iso_request.plate_set_label

        if iso_count >= self.iso_request.number_plates:
            msg = 'The ISOs have already been created.'
            self.add_error(msg)
            return

        for i in range(iso_count, self.iso_request.number_plates):
            layout_number = i + 1
            iso_label = self.ISO_LABEL_PATTERN % (ir_label, layout_number)
            ticket_creator = PoolCreationTicketGenerator(
            ...
            if ticket_number is None:
                ...
                LibraryCreationIso(ticket_number=ticket_number,
                             layout_number=layout_number,
                             label=iso_label,
...
class PoolCreationTicketWorklistUploader(BaseTracTool):
    """
    Uses the worklist files the generated by the
    :class:`PoolCreationWorklistWriter` and sends them to the ticket
    ...
    def __init__(self, pool_creation_iso, file_map,
                 logging_level=logging.WARNING, add_default_handlers=False):
        """
        Constructor:

        :param pool_creation_iso: The pool creation ISO the worklists
        ...
        #: The pool creation ISO the worklists belong to (also contains
        #: the ticket ID).
    ...
    def __check_input(self):
        ...
        self._check_input_class('pool creation ISO',
    ...
    def __prepare_and_submit(self):
        ...
        Submits the request.
        """
        fn = self.FILE_NAME % (self.pool_creation_iso.iso_request.\
                               plate_set_label,
                               self.pool_creation_iso.layout_number)
        attachment = AttachmentWrapper(content=self.file_map,
                                       file_name=fn,
                                       description=self.DESCRIPTION)
        ticket_id = self.pool_creation_iso.ticket_number
...
class PoolCreationStockTransferReporter(StockTransferReportUploader):

    EXECUTOR_CLS = PoolCreationExecutor
    ...
    def _get_plate_str(self):
        """
        The plate string looks different, we use new ISO sample stock rack
        (instead of the preparation plate).
        """
        rack = self.executor.entity.iso_sample_stock_racks[0].rack
        rack_str = '%s' % (rack.barcode)
        return rack_str

    def _get_log_file_writer(self):
        """
        For pool creation ISOs we use a special writer, the
        :class:`PoolCreationStockLogFileWriter`.
        """
        writer = PoolCreationStockLogFileWriter(log=self.log,
                    library_layout=self._working_layout,
                    executed_worklists=self._executed_worklists)
        return writer



class PoolCreationStockLogFileWriter(CsvWriter):
    """
    ...
    **Return Value:** file stream (CSV format)
    """
    NAME = 'Pool Creation Stock Transfer Log File Writer'
    ...
    def __init__(self, library_layout, executed_worklists, log):
        """
        Constructor:
    ...
    def __get_sorted_executed_transfers(self, executed_transfers,
        ...
        for et in executed_transfers:
            ...
            lib_pos = self.library_layout.get_working_position(rack_pos)
            if lib_pos is None:

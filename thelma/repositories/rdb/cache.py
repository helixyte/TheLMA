"""
thelma.repositories.rdb.cache

NP

Based on the beaker_caching example from the standard SQLAlchemy 0.6.1
distribution.
...
class CachingQueryFactory(object):

    def __init__(self, cache_manager):
        self.__cache_manager = cache_manager

    def __call__(self, entities, session=None):
        return CachingQuery(entities, session, self.__cache_manager)
...
class CachingQuery(Query):
    ...
    def __init__(self, entities, session, cache_manager):
    ...
    def __iter__(self):
        ...
        if hasattr(self, '_cache_parameters'):
            cache, cache_key = self._get_cache_parameters(self)
    ...
    # Accessing protected attributes of query parameter.
    # pylint: disable=W0212
    def _get_cache_parameters(self, query):
        ...
        if cache_key is None:
            ...
            args = self._params_from_query(query)
            cache_key = " ".join([str(x) for x in args])

        # get cache
        cache = self._cache_manager.get_cache_region(namespace, region)
    ...
    def _params_from_query(self, query):
...
class _CacheMapperOption(MapperOption):
    # Accessing protected attributes of query parameter.
    # pylint: disable=W0212
    def _set_cache_parameters(self, query, region, namespace, cache_key):
...
class RelationshipCache(_CacheMapperOption):
    ...
    def __init__(self, region, namespace, attribute):
        ...
        """
        _CacheMapperOption.__init__(self)
        self.region = region
...
class FromCache(_CacheMapperOption):
    ...
    def process_query(self, query):
        ...
        self._set_cache_parameters(query, self.region, self.namespace,
                                   self.cache_key)
